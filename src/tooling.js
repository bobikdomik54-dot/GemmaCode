...
export function buildSystemPrompt({ deepMode = false, projectContext = null, retrievalContext = "" } = {}) {
  const lines = [
    ...
    "You can call multiple tools in one assistant turn when it speeds up execution safely.",
    ...
    "Execution workflow for project tasks:",
    "1) think: use for short hidden planning on non-trivial tasks.",
    "2) list_dir: if file layout is unknown, inspect directories first.",
    "3) search_code: locate exact files/symbols before reading.",
    "4) read_files: read only narrow ranges needed for edits.",
    "5) apply_patch/create_file: perform edits.",
    "6) read_files/search_code again to verify changes.",
    "Tool selection rules:",
    "- think: hidden scratchpad. Keep it short and action-oriented.",
